...
function nightscoutSource (opts, axios) {
  ...
  var impl = {
    ...
    dataFromSesssion(session, last_known) {
      ...
      // Note: Some Nightscout instances don't support MongoDB-style find queries
      // So we fetch more entries and filter client-side
      var query = { count: Math.max(count, 288) }; // At least 1 day of data (288 * 5min)
      var dataUrl = '/api/v1/entries.json';
      var headers = { 'x-special-foo': 'special' };
      if (session.bearer) {
        headers['Authorization'] = ['Bearer', session.bearer].join(' ');
      }
      console.log("FETCHING DATA from", last_glucose_at.toISOString(), "count:", query.count);
      return http.get(dataUrl, { params: query, headers }).then((resp) => {
        var all_data = resp.data || [];
        // Filter client-side for entries after last_known
        var filtered = all_data.filter((entry) => {
          var entryDate = new Date(entry.dateString || entry.date);
          return entryDate > last_glucose_at;
        });
        console.log("✅ FETCHED DATA:", all_data.length, "total,", filtered.length, "new entries");
        return filtered;
      }).catch(err => {
        console.error("❌ dataFromSesssion FAILED:", err.message || err);
        throw err;
      });
    },
