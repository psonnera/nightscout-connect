  ...
  var impl = {
    authFromCredentials ( ) {
      ...
      return http.post(Defaults.Login, payload).then((response) => {
        debug("LIBRE LINKUP AUTH", response.headers, response.data);
    ...
    sessionFromAuth (auth) {
      ...
      return http.get(Defaults.Connections, { headers }).then((resp) => {
        debug("CONNECTIONS RESPONSE FROM LIBRELINKUP", resp.status, resp.headers, resp.data);
        ...
        debug("LIBRE SESSION FROM AUTH", result);
        return result;
    ...
    dataFromSesssion (session, last_known) {
      ...
      return http.get(graph_url, { headers }).then((resp) => {
        debug("RECEIVED LIBRE GRAPH DATA", resp.status, resp.headers, resp.data);
        return resp.data;
      });
    },
    transformGlucose (payload, last_known) {
      ...
      var devicestatus = [ ];
      var profiles = [ ];
      debug("TRANSFORMING LIBRE BATCH", batch);
